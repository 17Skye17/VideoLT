...
class Scale(torch.nn.Module):
    def __init__(self, num_class=1004):
        super(Scale, self).__init__()        
        self.p = torch.nn.Parameter(torch.ones(num_class))
...
def main():
   ...
   # optimizer = torch.optim.SGD(params_to_update, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay) 
    ...
    params_to_update = []
    for name, param in scales.named_parameters():
        if param.requires_grad == True:
            params_to_update.append(param)
            print ('\t', name)
    ...
    if args.evaluate:
        validate(val_loader, model,epoch, log_training, indices)
        exit(0)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(train_loader, model, epoch, log_training, indices, scales)
        
        if (epoch + 1) % args.eval_freq == 0 or epoch == args.epochs - 1:
            acc1, acc5, mAP = validate(val_loader, model, epoch, log_training, indices, scales)
...
def train(loader, model, epoch, log, indices, scales):
    ...
    for i, (vid, feature, target) in enumerate(loader):
        ...
        if args.mixup == 'clipmix':
            mixed_input, mixed_target = getattr(Mixup, args.mixup)(feature, target, args.clip_length)
            output = model(mixed_input)
            loss = criterion(output, mixed_target)
        elif args.mixup == 'mixup':
            gamma = np.random.beta(1.0, 1.0)
            mixed_input, mixed_target = getattr(Mixup, args.mixup)(feature, target, gamma)
            output = model(mixed_input)
            loss = criterion(output, mixed_target)
        else:
            output = model(feature)
            output = scales(output)
...
def validate(loader, model, epoch, log, indices, scales):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
